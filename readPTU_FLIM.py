...
#             num_tcspc_channel    = floor(sync_rate/tcspc_bin_resolution)+1
        ...
        FrameSyncVal    = sync[np.where(special == FrameMarker)]#this is the sync values for when there is a frame clock
        StartSyncVal    = sync[np.where(special == LineStartMarker)]
        EndSyncVal    = sync[np.where(special == LineStopMarker)]
        
        num_of_Frames   = FrameSyncVal.size#the number of frames is just the amount of elements in this array, however many frame clocks there were
        read_data_range = np.where(sync == FrameSyncVal[num_of_Frames-1])[0][0]# FSV[num of frames-1] gives the sync value of the last frame clock, where sync is
                                                                                # equal to this tells us the record at which the frames stop
